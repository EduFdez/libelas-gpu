...
int main(int argc, char** argv) {
  ...
  for (auto it_l = allimages_l.begin(), it_r = allimages_r.begin();
    ...
    Mat imgLeft = imread(left_name, cv::IMREAD_COLOR);
    Mat imgRight = imread(right_name, cv::IMREAD_COLOR);
    ...
    cvtColor(imgLeft, imgLGray, cv::COLOR_BGR2GRAY);
    cvtColor(imgRight, imgRGray, cv::COLOR_BGR2GRAY);
